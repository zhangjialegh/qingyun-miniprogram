			 ...
			 :style="{ width: boxWidth, height: boxHeight, backgroundColor: styles.backgroundColor }" class="uni-fab__content">
				...
				<view v-for="(item, index) in content" :key="index" :class="{ 'uni-fab__item--active': isShow }" class="uni-fab__item"
				 @click="_onItemClick(index, item)">
				    <text class="uni-fab__item-font iconfont" :class="[item.font]" :style="{ color: item.active ? styles.selectedColor : styles.color }" v-if="item.font"></text>
					<image v-else :src="item.active ? item.selectedIconPath : item.iconPath" class="uni-fab__item-image" mode="widthFix" />
